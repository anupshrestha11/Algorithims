...
public class AStarAlgorithm {
    ...
    public void search() {
        ...
        //the algorithm terminates when there is no item left in the openSet
        while (!openSet.isEmpty()) {

            //poll: returns the node with the smallest f=h+g value
            Node currentNode = openSet.poll();
            System.out.println(currentNode + " Predecesor is: " + currentNode.getPredecessor());

            //if we find the terminal state we've done
            if (currentNode.equals(finalNode)) return;

            //of course we have to update sets
            openSet.remove(currentNode);
            closedSet.add(currentNode);

            //visit all the neighbours of the actual node
